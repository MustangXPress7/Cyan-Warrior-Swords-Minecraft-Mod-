...
public class SYNERGY_TOTEM extends Item implements IHasModel{
    public SYNERGY_TOTEM(String name)
    {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if(playerIn.isSneaking()){
            ...
            ItemStack active_totemStack = new ItemStack(ModItems.ActiveSynergyTotem,1);
            active_totemStack.addEnchantment( ModEnchantments.DwEnchantment,1);

            if(xEnc.containsKey(ModEnchantments.InhEnchantment)){
                active_totemStack.addEnchantment(ModEnchantments.InhEnchantment,1);
            }

            ITextComponent chatMessage=new TextComponentTranslation("chat.usage.activation.dw");
            if(!worldIn.isRemote){
                playerIn.sendMessage(chatMessage);
            }
            playerIn.getCooldownTracker().setCooldown(active_totemStack.getItem(), 40);
            return new ActionResult<>(EnumActionResult.SUCCESS, active_totemStack);
        }
    }
