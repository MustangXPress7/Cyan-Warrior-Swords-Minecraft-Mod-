...
public class ItemConfig {
    ...
    private static final String PATH = FMLPaths.CONFIGDIR.get().resolve("cwsr-config.json").toString();
    private static final File DBFILE = new File(PATH);
    public static boolean initialized = false;
    private static JsonObject db = new JsonObject();

    private static String format(String message) {
        return String.format("[%s] %s", CyanWarriorSwordsReduxMod.MOD_ID, message);
    }
    ...
    public static void load() {
        ...
        try {
            ...
            CyanWarriorSwordsReduxMod.LOGGER.error(e.getMessage());
            CyanWarriorSwordsReduxMod.LOGGER.error(format("Unable to load configuration file!"));
        }
    ...
    private static void save() {
        try {
            ...
            CyanWarriorSwordsReduxMod.LOGGER.error(e.getMessage());
            CyanWarriorSwordsReduxMod.LOGGER.error(format("Unable to save configuration file!"));
        }
    }
    ...
    private static void convert(int version) {
        ...
        CyanWarriorSwordsReduxMod.LOGGER.info(format("Converted outdated config."));
    }
    ...
    private static void repair() {
        ...
        if (repaired > 0) {
            CyanWarriorSwordsReduxMod.LOGGER.info(format("Repaired " + repaired + " config properties"));
        }
