...
public class ICE_SWORD extends SWORD_CWSR {
    ...
    public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new TranslationTextComponent("tooltip.cwsr.ice_sword"));
    }
    ...
    public ActionResult<ItemStack> eventRC(World world, PlayerEntity entity, Hand handIn, ItemStack OffHandItem) {
        ...
        for (int i = 0; i <= e.size() - 1; i++) {
            ...
            if (em instanceof LivingEntity && !(em instanceof ArmorStandEntity)){
                ...
                ((LivingEntity) em).addEffect(new EffectInstance(Effects.MOVEMENT_SLOWDOWN,100,4));
                ...
                double d1 = MathHelper.lerp(d0, em.xo, em.getX()) + (((LivingEntity) em).getRandom().nextDouble() - 0.5D) * (double)em.getBbWidth() * 2.0D;
                double d2 = MathHelper.lerp(d0, em.yo, em.getY()) + ((LivingEntity) em).getRandom().nextDouble() * (double)em.getBbHeight();
                double d3 = MathHelper.lerp(d0, em.zo, em.getZ()) + (((LivingEntity) em).getRandom().nextDouble() - 0.5D) * (double)em.getBbWidth() * 2.0D;
...
currentSword.hurtAndBreak(SwordConfig.ICE_SWORD_USE_COST.get(), entity,playerEntity -> {
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker){
        target.addEffect(new EffectInstance(Effects.DIG_SLOWDOWN,SwordConfig.ICE_SWORD_HIT_TK.get(),3));
